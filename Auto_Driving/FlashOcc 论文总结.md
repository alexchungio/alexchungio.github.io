...
占用头BEV编码器，由多层卷积网络或复杂的多尺度特征融合模块组成，用于提升BEV特征的表示性能。

多尺度融合具有更优的全局感受野，同时提供对局部细节更精细的表征。

### BEV解码器

占用头的BEV解码器，利用编码后的BEV特征，解码 OCC 占用预测。其中，BEV解码器的关键模块是**通道到高度模块（Channel-to-Height module）**。

通道到高度模块，通道到高度模块将BEV特征沿通道维度执行简单的变形操作，用来将2D的BEV特征，提升到3D的OCC占用特征。主要分为两个步骤

1. 首先，利用全连接层对特征的通道维度做变换，将BEV特征转换为$B \times C \times W \times H$的形状，其中$C=C^* \times Z$, $W$,$H$, $Z$分别代表 $x$,$y$,$z$维度在3D空间对应的数量。
2. 然后，将通道转换后的特征变形为$B \times C\times Z \times W \times H$形状的的占用logits。
